...
if (form && formSuccess) {
    ...
    /* Supabase client — only created here, inside the contact-page
       guard, so pages without the Supabase library (i.e. every page
       except contact.html) never touch it and never throw. */
    ...
    if (
        ...
        console.warn(
            ...
            "SUPABASE_ANON_KEY in contact.html. Falling back to " +
            "saving submissions locally in this browser only."
        );
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if (supabaseClient) {
            ...
            /* Fallback so the form still works before Supabase is
               configured — saved only in this browser, not shared. */

            const existing = JSON.parse(
                localStorage.getItem("adeyemiGalleryEnquiries") || "[]"
            );

            existing.push({ ...entry, submittedAt: new Date().toISOString() });

            localStorage.setItem(
                "adeyemiGalleryEnquiries",
